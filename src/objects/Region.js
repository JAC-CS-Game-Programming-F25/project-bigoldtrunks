...
export default class Region {
  ...
  isEntityReadyToTakeDamage(entity) {
    return (
      ...
      this.player.stateMachine.currentState.name !=
      ...
      for (let i = 0; i < def.count; i++) {
        ...
        do {
          ...
          if (def.type === CreatureType.BigBoss) {
            const x = getRandomPositiveInteger(80, 280);
            const y = getRandomPositiveInteger(40, 80);
            position = new Vector(x, y);
          } else {
            // Other creatures spawn in wider area
            const x = getRandomPositiveInteger(40, 300);
            const y = getRandomPositiveInteger(40, 120);
            position = new Vector(x, y);
          }
          attempts++;
        } while (
